...
var ReactBubbleChartD3 = (function () {
  function ReactBubbleChartD3(el) {
    ...
    this.html = d3.select(el).append("div").attr("class", "bubble-chart-text").style("position", "absolute").style("left", 0) // center horizontally
    .style("right", 0).style("margin-left", "auto").style("margin-right", "auto");
    this.legend = d3.select(el).append("svg").attr("class", "bubble-legend").style("overflow", "visible").style("position", "absolute");
  ...
  _createClass(ReactBubbleChartD3, [{
    ...
    value: function adjustSize(el) {
      ...
      this.bubble = d3.pack().size([this.diameter, this.diameter]).padding(3);
    }
    ...
    value: function update(el, props) {
      ...
      var color = d3.scaleQuantize().domain([props.fixedDomain ? props.fixedDomain.min : d3.min(data, function (d) {
        return d.colorValue;
      }), props.fixedDomain ? props.fixedDomain.max : d3.max(data, function (d) {
        return d.colorValue;
      })]).range(this.colorRange);

      // define a color scale for text town
      var textColor = d3.scaleQuantize().domain([props.fixedDomain ? props.fixedDomain.min : d3.min(data, function (d) {
        return d.colorValue;
      }), props.fixedDomain ? props.fixedDomain.max : d3.max(data, function (d) {
        return d.colorValue;
      ...
      var nodes = d3.hierarchy(data.length ? { children: data } : data).sum(function (d) {
        return d.value;
      });

      // assign new data to existing DOM for circles and labels
      var circles = this.svg.selectAll("circle").data(this.bubble(nodes).descendants(), function (d) {
        return "g" + d.data._id;
      });
      var labels = this.html.selectAll(".bubble-label").data(this.bubble(nodes).descendants(), function (d) {
        return "g" + d.data._id;
      });
      ...
      circles.transition().duration(duration).delay(function (d, i) {
        ...
        return d.selected ? _this2.selectedColor : color(d.data.colorValue);
      }).call(this.callback);
      ...
      labels.on("mouseover", this._tooltipMouseOver.bind(this, color, el)).transition().duration(duration).delay(function (d, i) {
        ...
        return d.selected ? _this2.selectedTextColor : textColor(d.data.colorValue);
      ...
      if (this.bubble(nodes).descendants().length) {
        ...
        circles.enter().filter(function (d) {
          ...
          var classConstant = d.children ? ["bubble"] : ["bubble leaf"];
          return [].concat(_toConsumableArray(d.data["class"] || []), classConstant).join(" ");
          ...
          return d.r * 0.2;
        }).style("opacity", 1).call(this.callback);
        // intialize new labels
        var node = labels.enter().append("div").attr("class", function (d) {
          var size;
          if (2 * d.r < _this2.smallDiameter) size = "small";else if (2 * d.r < _this2.mediumDiameter) size = "medium";else size = "large";
          return "bubble-label " + size;
        });
        if (props.safeLabel) node = node.text(function (d) {
          return d.data.displayText || d.data._id;
        });else node = node.html(function (d) {
          return d.data.displayText || d.data._id;
        });
    ...
    value: function _tooltipMouseOver(color, el, d, i) {
      ...
      var fill = color(d.data.colorValue);
      var backgroundColor = d3.rgb(d3.rgb(fill).r + 0.8 * (255 - d3.rgb(fill).r), d3.rgb(fill).g + 0.8 * (255 - d3.rgb(fill).g), d3.rgb(fill).b + 0.8 * (255 - d3.rgb(fill).b));
