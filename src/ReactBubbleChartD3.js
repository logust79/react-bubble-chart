...
export default class ReactBubbleChartD3 {
  constructor(el, props = {}) {
    this.legendSpacing =
      typeof props.legendSpacing === "number" ? props.legendSpacing : 3;
  ...
  configureLegend(el, props) {
    ...
    const legendHeight =
      this.colorLegend.length * (legendRectSize + this.legendSpacing) -
      this.legendSpacing;
    this.legend
  ...
  update(el, props) {
    ...
    this.colorLegend = colorLegend
      .slice(0)
      .reverse()
      .map(c => (typeof c === "string" ? { color: c } : c));
    ...
    const color = d3
      ...
      .domain([
        props.fixedDomain
          ? props.fixedDomain.min
          : d3.min(data, d => d.colorValue),
        props.fixedDomain
          ? props.fixedDomain.max
    ...
    const nodes = d3
      .hierarchy(data.length ? { children: data } : data)
      .sum(d => d.value);
    ...
    labels
      ...
      .style("opacity", 1)
      .style("color", d =>
        d.selected ? this.selectedTextColor : textColor(d.data.colorValue)
      )
      .attr("class", d => {
        var size;
    ...
    if (this.bubble(nodes).descendants().length) {
      ...
      circles
        ...
        .classed("selected", d => (d.selected ? true : false))
        .style("fill", d =>
          d.selected ? this.selectedColor : color(d.data.colorValue)
        )
        .transition()
      ...
      if (props.safeLabel)
        node = node.text(d => d.data.displayText || d.data._id);
  ...
  _tooltipMouseOver(color, el, d, i) {
    ...
    for (var { css, prop, display } of this.tooltipProps) {
      this.tooltip
        .select("." + css)
        .html((display ? display + ": " : "") + d.data[prop]);
    }
